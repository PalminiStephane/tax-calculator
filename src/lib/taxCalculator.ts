...
export function calculateTaxes(config: TaxConfig): TaxResult {
  ...
  // ── 5. Total URSSAF ──────────────────────────────────────────────────────────
  const totalURSSAF = cotisationsSociales + cfp + tcciTcma + versementLiberatoire
  ...
  if (isAboveTvaThresholdMajore && !isTvaApplicable) {
    ...
    warnings.push({
      type: 'warning',
      message: `Votre CA dépasse le seuil de base de franchise TVA (${formatCurrency(tvaSeuils.base)}). Si votre CA de l'année précédente était aussi dépassé, la TVA s'applique.`,
    })
  }
